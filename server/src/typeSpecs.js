// server/src/typeSpecs.js
// Type specifications for all 20 service types across 5 categories.
// Ported from data/types.ts for use in the live simulator.

// --- helpers ---
const rint = (r, lo, hi) => Math.floor(r() * (hi - lo + 1)) + lo;
const rfloat = (r, lo, hi, dp = 1) => Number((r() * (hi - lo) + lo).toFixed(dp));
const pad = (n, w = 3) => String(n).padStart(w, '0');

// Severity to weight/sigmaKm mapping
export const SEVERITY_MAP = {
  p1: { weight: 5, sigmaKm: 10 },
  p2: { weight: 4, sigmaKm: 10 },
  p3: { weight: 2, sigmaKm: 5 },
  p4: { weight: 1, sigmaKm: 5 },
};

export const SEVERITIES = ['p1', 'p2', 'p3', 'p4'];
export const SEVERITY_WEIGHTS = [0.15, 0.25, 0.35, 0.25]; // distribution

export const REPORTED_BY = ['customer', 'alarm', 'field-tech', 'proactive'];
export const REPORTED_BY_WEIGHTS = [0.35, 0.30, 0.20, 0.15];

export const IMPACT_SCOPES = ['single', 'segment', 'metro', 'region'];
...
export const TYPE_SPECS = {
  ...
  fiber: {
    ...
    phrasings: [
      ...
      'backhoe strike confirmed on the route',     // literal "backhoe" - capped at 8%
  ...
  satellite: {
    ...
    template: (f, p, n) =>
      `${p} on ${f.terminalId}, ${f.beamId}. SNR ${f.snrDb}dB against ${f.expectedSnrDb} nominal. Elevation ${f.elevationDeg} deg, ${f.orbitClass}. ${n} terminals on the beam.`,
    digRelated: false,
    crew: 'satellite',
    parts: [],
    costBase: 1100,
...
export const ALL_TYPES = Object.keys(TYPE_SPECS);
